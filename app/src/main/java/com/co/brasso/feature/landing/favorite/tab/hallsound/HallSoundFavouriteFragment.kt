...
class HallSoundFavouriteFragment :
    BaseFragment<HallSoundFavouriteView, HallSoundFavouritePresenter>(), HallSoundFavouriteView {

    private var binding: FragmentConductorFavouriteBinding? = null
    private var hallSoundFavouriteAdapter: ConductorFavouriteAdapter? = null
    private var favouriteList: MutableList<FavouriteResponse>? = null
    var searchSlug: String? = ""
    var fragmentType: String? = Constants.hallSound
    private var isFirst = true
    var type: String? = Constants.hallSound
    ...
    override fun createPresenter() = HallSoundFavouritePresenter()
    ...
    private fun initRecyclerView() {
        ...
        hallSoundFavouriteAdapter = ConductorFavouriteAdapter(favouriteList ?: mutableListOf(), {
        ...
        binding?.rcvConductorFavourite?.adapter = hallSoundFavouriteAdapter
    }
    ...
    private fun removeFromFavouriteList(position: Int?) {
        ...
        hallSoundFavouriteAdapter?.notifyItemRemoved(position ?: 0)
    ...
    fun setAdapter(favouriteList: List<FavouriteResponse>?) {
        if (favouriteList.isNullOrEmpty()) {
            ...
            hallSoundFavouriteAdapter?.notifyDataSetChanged()
        }
    }
    ...
    override fun onResume() {
        ...
        if (selectedPos != null) {
            ...
            hallSoundFavouriteAdapter?.notifyItemRemoved(selectedPos ?: 0)
            if (favouriteList.isNullOrEmpty()) {
                showErrorLayout()
            }
            selectedPos = null
