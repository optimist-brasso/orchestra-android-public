...
class ListSongOrchestraFragment :
    BaseFragment<ListSongOrchestraView, ListSongOrchestraPresenter>(), ListSongOrchestraView {
    ...
    var tempList: MutableList<HallSoundResponse>? = null
    ...
    override fun createPresenter() = ListSongOrchestraPresenter()

    override fun onResume() {
        super.onResume()
        if (selectedPos != null) {
            orchestraList?.get(selectedPos ?: 0)?.isConductorFavourite =
                !(orchestraList?.get(selectedPos ?: 0)?.isConductorFavourite ?: false)
            selectedPos = null
        }
        orchestraSearchAdapter?.notifyDataSetChanged()
    }
