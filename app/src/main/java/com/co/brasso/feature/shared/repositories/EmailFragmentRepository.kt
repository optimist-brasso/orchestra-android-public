...
object EmailFragmentRepository : BaseRepository() {

    fun getOTP(compositeDisposable: CompositeDisposable?, emailVerification: EmailVerification?): Single<EmailFragmentResponse> = Single.create { e ->
        apiService?.getOTP(emailVerification?.toJson())
            .getSubscription()
            ?.subscribe({
                if (it.isSuccessful) {
