package com.co.brasso.utils.customView.imageCropper
...
import com.co.brasso.utils.customView.imageCropper.common.CommonVersionCheck
import com.co.brasso.utils.util.BitmapUtils
import java.util.*
import kotlin.math.*
...
@JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null) : View(context, attrs) {
     ...
     */
    private fun setCropCornerRadius(cornerRadius: Float) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (mCropWindowHandler.showGuidelines()) {
            ...
            if (guidelines == CropImageView.Guidelines.ON) drawGuidelines(canvas)
            else if (guidelines == CropImageView.Guidelines.ON_TOUCH && mMoveHandler != null) drawGuidelines(
                canvas
            )
        }
    ...
    private fun drawBorders(canvas: Canvas) {
        if (mBorderPaint != null) {
            ...
            when (cropShape) {
                ...
                CropImageView.CropShape.RECTANGLE_VERTICAL_ONLY,
                CropImageView.CropShape.RECTANGLE_HORIZONTAL_ONLY,
                CropImageView.CropShape.RECTANGLE -> canvas.drawRect(rect, mBorderPaint!!)
                // Draw circular crop window border
                CropImageView.CropShape.OVAL -> canvas.drawOval(rect, mBorderPaint!!)
                else -> throw IllegalStateException("Unrecognized crop shape")
            }
        }
    }
    ...
    private fun drawCorners(canvas: Canvas) {
        if (mBorderCornerPaint != null) {
            ...
            val w: Float = when (cropShape) {
                ...
                CropImageView.CropShape.RECTANGLE_VERTICAL_ONLY,
                CropImageView.CropShape.RECTANGLE_HORIZONTAL_ONLY,
                CropImageView.CropShape.RECTANGLE -> cornerWidth / 2 + mBorderCornerOffset
                CropImageView.CropShape.OVAL -> cornerWidth / 2
                else -> throw IllegalStateException("Unrecognized crop shape")
            }
            val rect = mCropWindowHandler.getRect()
            rect.inset(w, w)
    ...
    private fun drawCornerShape(
        ...
        when (cornerShape) {
            ...
            CropImageView.CropCornerShape.RECTANGLE -> drawLineShape(
                ...
                Log.d("brassoError", "drawCornerShape: Failed")
            }
        }
    }
